...
traindata, validdata, testdata = get_dataloader(args.dataset_path,  robust_test=False, data_type=args.dataset)
...
encoders = [Identity().cuda() for _ in d_modalities]
head = Sequential(Transformer(out_dim, 300).cuda(), MLP(300, 128, 1)).cuda()
...
testtimes = []
accs = []
saved_model = '{}_models/{}_ef_transformer_{}.pt'.format(args.dataset, args.dataset, ''.join(list(map(str, modalities))))
for _ in range(5):
    traintime, mem, num_params = train(encoders, fusion, head, traindata, validdata, 50, task="regression", optimtype=torch.optim.AdamW, is_packed=True,lr=1e-4, save=saved_model, weight_decay=0.01, objective=torch.nn.L1Loss(), modalities=modalities)
    ...
    testtime, acc = test(model, testdata, 'affect', is_packed=True,
