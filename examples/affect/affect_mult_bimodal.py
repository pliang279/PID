...
train(encoders, fusion, head, traindata, validdata, 150, task="regression", optimtype=torch.optim.AdamW, early_stop=False, is_packed=False, lr=1e-3, clip_val=1.0, save='sarcasm_mult12_best.pt', weight_decay=0.01, objective=torch.nn.L1Loss(), modalities=modalities)

print("Testing:")
model = torch.load('sarcasm_mult12_best.pt').cuda()
