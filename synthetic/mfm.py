...
decoders = [Linear(args.n_latent, input_dim).to(device) for input_dim in input_dims]
fusion = Sequential2(Concat(), MLP(len(args.modalities)*args.hidden_dim, args.n_latent, args.n_latent//2)).to(device)
intermediates = [MLP(args.hidden_dim, args.n_latent//2, args.n_latent//2).to(device) for _ in args.modalities]
head = MLP(args.n_latent//2, 20, args.num_classes).to(device)
argsdict = {'decoders': decoders, 'intermediates': intermediates}
additional_modules = decoders+intermediates
objective = MFM_objective(2.0, [torch.nn.MSELoss(), torch.nn.MSELoss(), torch.nn.MSELoss()], [args.weight, args.weight, args.weight])

# Training
train(encoders, fusion, head, traindata, validdata, args.epochs, additional_modules, objective=objective, objective_args_dict=argsdict, optimtype=torch.optim.AdamW, is_packed=False, lr=args.lr, save=args.saved_model, weight_decay=args.weight_decay)
