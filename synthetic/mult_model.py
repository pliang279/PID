...
from fusions.mult.modules.transformer import TransformerEncoder
...
class MULTModel(nn.Module):
    ...
    def __init__(self, n_modalities, n_features, hyp_params=DefaultHyperParams):
        ...
        self.proj = [nn.Linear(n_features[i], self.embed_dim) for i in range(n_modalities)]
    ...
    def forward(self, x):
        ...
        proj_x = [torch.unsqueeze(self.proj[i](x[i]), dim=0) for i in range(self.n_modalities)]
