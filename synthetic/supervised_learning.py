...
def train(
        ...
        track_complexity=True, pretrain=None):
    """
...
def single_test(model, test_dataloader, criterion=nn.CrossEntropyLoss(), task="classification", auprc=False, input_to_float=True, cluster=None, sub_acc=None, save_preds=None):
    ...
    with torch.no_grad():
        ...
        for (i, j) in enumerate(test_dataloader):
            model.eval()
            out = model([_processinput(i).float().to(device) for i in j[:-1]])
        ...
        if auprc:
            print("AUPRC: "+str(AUPRC(pts)))
        truths = true.cpu().numpy().reshape(-1,)
        preds = pred.cpu().numpy().reshape(-1,)
        false_idx = truths != preds
        if sub_acc:
            false_ids = np.where(truths != preds)[0]
            print("Misclassified example:", ' '.join([str(i) for i in false_ids]))
        if cluster:
            data = (cluster['0'][false_idx,:], cluster['1'][false_idx,:], cluster['label'][false_idx,:])
            P, maps = convert_data_to_distribution(*data)
            get_measure(P)
        acc = accuracy(true, pred)
        print("acc: "+str(acc))
        if sub_acc:
            print("sub_acc:", accuracy(true[sub_acc], pred[sub_acc]))
        if save_preds:
...
def test(
        model, test_dataloaders_all, dataset='default', method_name='My method', is_packed=False, criterion=nn.CrossEntropyLoss(), task="classification", auprc=False, input_to_float=True, no_robust=False, cluster=None, sub_acc=None, save_preds=None):
    """
    ...
    """
    if no_robust:
        return single_test(model, test_dataloaders_all, criterion, task, auprc, input_to_float, cluster=cluster, sub_acc=sub_acc, save_preds=save_preds)
